#!/usr/bin/env python3
"""Run untuned Qwen3.5 9B as a configurable blinded matrix judge."""
...
def main() -> None:
    ...
    output_repo = os.environ.get(
        "QWEN_JUDGE_OUTPUT_REPO", "vamsee9201/qwen35-9b-local-hybrid-rag-lora"
    )
    ...
    subprocess.run([
        "swift", "infer", "--model", MODEL, "--model_revision", BASE_REVISION,
        "--use_hf", "true",
    ...
    (OUTPUT / "manifest.json").write_text(json.dumps({
        "model": MODEL, "base_revision": BASE_REVISION,
        "records": len(rows), "elapsed_seconds": time.perf_counter() - started,
        "max_new_tokens": max_new_tokens,
    }, indent=2) + "\n")
    HfApi().upload_folder(
        repo_id=output_repo, repo_type="model",
        folder_path=OUTPUT, path_in_repo=upload_subdir,
        commit_message="Upload blinded matrix Qwen judge output",
    )
