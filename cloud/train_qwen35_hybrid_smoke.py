#!/usr/bin/env python3
"""Run the Qwen3.5 9B Vertex-hybrid LoRA compatibility and reload gate."""
...
OUTPUT_DIR = Path("/tmp/qwen35-hybrid-smoke")
...
def main() -> None:
    ...
    run([
        ...
        "--val_dataset_sample", "3", "--result_path", str(inference), "--max_batch_size", "1",
    ...
    print("HYBRID_SMOKE_TEST_COMPLETE", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"HYBRID_SMOKE_TEST_FAILED: {type(exc).__name__}: {exc}", file=sys.stderr, flush=True)
        raise
