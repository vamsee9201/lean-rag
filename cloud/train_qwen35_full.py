#!/usr/bin/env python3
"""Run the full one-epoch Qwen3.5 9B RAG LoRA fine-tune on Hugging Face Jobs."""
...
def main() -> None:
    ...
    checkpoint = Path(best_checkpoint) if best_checkpoint else latest_checkpoint
    ...
    (OUTPUT_DIR / "full_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    HfApi().upload_folder(
        repo_id=OUTPUT_REPO,
        repo_type="model",
        folder_path=OUTPUT_DIR,
        path_in_repo="run1",
        commit_message="Upload Qwen3.5 9B RAG LoRA full one-epoch run",
    )
    print("FULL_TRAINING_COMPLETE", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"FULL_TRAINING_FAILED: {type(exc).__name__}: {exc}", file=sys.stderr, flush=True)
        raise
