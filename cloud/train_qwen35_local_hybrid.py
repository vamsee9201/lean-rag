#!/usr/bin/env python3
"""Train a fresh Qwen3.5 9B LoRA adapter on tuned-local-hybrid contexts."""
...
OUTPUT_DIR = Path("/tmp/qwen35-local-hybrid-full")
...
def main() -> None:
    ...
    HfApi().upload_folder(
        ...
        commit_message="Upload Qwen3.5 9B local-hybrid RAG LoRA run",
    )
    print("LOCAL_HYBRID_TRAINING_COMPLETE", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"LOCAL_HYBRID_TRAINING_FAILED: {type(exc).__name__}: {exc}", file=sys.stderr, flush=True)
        raise
