#!/usr/bin/env python3
"""Run the paid Qwen3.5 9B LoRA compatibility gate on Hugging Face Jobs."""
...
TRAIN_DATA = Path("/mnt/data/pilot/train_sft.jsonl")
VALIDATION_DATA = Path("/mnt/data/pilot/validation_sft.jsonl")
OUTPUT_DIR = Path("/tmp/qwen35-rag-smoke")
...
def main() -> None:
    ...
    HfApi().upload_folder(
        ...
        path_in_repo="smoke",
        commit_message="Upload Qwen3.5 9B RAG LoRA smoke test",
    )
    print("SMOKE_TEST_COMPLETE", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"SMOKE_TEST_FAILED: {type(exc).__name__}: {exc}", file=sys.stderr, flush=True)
        raise
